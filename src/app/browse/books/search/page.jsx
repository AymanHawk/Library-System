...
function Search() {
  ...
  const genre = ["dystopian", "comics", "literary", "nature", "science", "economics", "manga", "essays", "religion", "short stories", "design", "contemporary", "self help", "childrens", "sports", "technology", "computer science", "romance", "education", "sociology", "horror", "philosophy", "history", "graphic novel", "science fiction", "thriller", "mathematics", "fiction", "classics", "speculative fiction", "art", "magical realism", "cookbook", "food and drink", "race", "autobiography", "video games", "nonfiction", "fantasy", "true crime", "lgbtqia+", "feminism", "poetry", "gender", "psychology", "erotica", "middle grade", "memoir", "biography", "mystery", "young adult", "politics", "crime", "music", "health", "business", "historical", "travel"]
  const theme = ["adventurous", "challenging", "dark", "emotional", "funny", "hopeful", "informative", "inspiring", "lighthearted", "mysterious", "reflective", "relaxing", "sad", "tense"]
  const pace = ["fast", "medium", "slow"]
  const format = ["Paperback", "Audio", "Hardcover", "Digital"]
  const language = ["Slovak", "Norwegian", "Hindi", "Български", "Polski", "Vietnamese", "Mandarin", "Swedish", "Italiano", "Latvian", "Latin", "Mandarine", "Slovenian", "Malay", "Scottish gaelic", "Dutch", "Afrikaans", "Danish", "Chineset", "Romanian", "German", "Bulgarian", "Tamil", "Hungarian", "Ukrainian", "Japanese", "Catalan", "Tagalog", "Português", "Estonian", "Russian", "Serbian", "Finnish", "Ukranian", "Basque", "Lithuanian", "Spanish", "Albanian", "Indonesian", "Croatian", "Czech", "Maltese", "Scots", "Svenska", "Engliah", "Greek", "Norwegian bokmål", "Galician", "Polish", "Arabic", "French", "Bengali", "Filipino", "Icelandic", "English", "Korean", "Italian", "English translated", "Welsh", "Malayalam", "Nepali", "Français", "Deutsch", "Turkish", "Belarusian", "Chinese", "Hebrew", "Portuguese", "Swahili"]
  ...
  const handleClickOutside = (event) => {
    ...
    if (
      ...
      (paceRef.current && !paceRef.current.contains(event.target))
    ) {
      setPaceDrop(false);
    } if (
  ...
  return (
    <div className='2xl:w-[1400px] text-primary xl:w-[1200px] lg:w-[1000px] norm:w-[750px] md:w-[600px] sm:w-[450px] w-[340px] xs:w-[275px] mx-auto'>
      ...
      <div className='flex'>
        <div className='2xl:w-[250px] text-primary xl:w-[200px] lg:w-[200px] norm:w-[200px] md:w-[150px] sm:w-[150px] w-[100px] xs:w-[100px]'>
          <h2 className='text-center text-2xl'>Filters</h2>
          <div className='mb-5'>
            <div>
              <div className='flex gap-2 items-center cursor-pointer' onClick={handleGenreDrop} ref={genreRef}>
                <h3 className='text-xl relative'>Genres</h3>
                <Image src={drop} width={15} height={10} alt='drop' />
              </div>
              <div className={(genreDrop ? `` : `hidden`) + ` flex flex-col gap-1 border-secondary absolute z-50 border-[1px] rounded-md py-2 bg-background 2xl:w-[250px] xl:w-[200px] lg:w-[200px] norm:w-[200px] md:w-[150px] sm:w-[150px] w-[100px] xs:w-[100px]`}>
                <div className='border-b-[1px] h-[47px] border-secondary pb-2 px-2 flex gap-2 py-1 overflow-x-auto no-scrollbar'>
                  {filters.genre.map((g) => (
                    <h2 key={g} className='border-secondary capitalize text-secondary border-[1px] rounded-md p-1'>{g}</h2>
                ...
                <div className='flex flex-wrap p-2 h-[150px] overflow-y-auto no-scrollbar'>
                  {genre.map((genre) => (
                    <div key={genre} >
                      <label className={(filters.genre.includes(genre) ? `text-secondary hover:text-white` : 'hover:text-secondary cursor-pointer text-white') + ` mx-1 capitalize`} htmlFor={genre}>
                        {genre}
                      </label>
                      <input
                        type="checkbox"
            ...
            <div>
              <div className='flex gap-2 items-center cursor-pointer' onClick={handleThemeDrop} ref={themeRef}>
                <h3 className='text-xl relative'>Themes</h3>
                <Image src={drop} width={15} height={10} alt='drop' />
              </div>
              <div className={(themeDrop ? `` : `hidden`) + ` flex flex-col gap-1 border-secondary absolute z-40 border-[1px] rounded-md py-2 bg-background 2xl:w-[250px] xl:w-[200px] lg:w-[200px] norm:w-[200px] md:w-[150px] sm:w-[150px] w-[100px] xs:w-[100px]`}>
                <div className='border-b-[1px] h-[47px] border-secondary pb-2 px-2 flex gap-2 py-1 overflow-x-auto no-scrollbar'>
                  {filters.theme.map((t) => (
                    <h2 key={t} className='border-secondary capitalize text-secondary border-[1px] rounded-md p-1'>{t}</h2>
                ...
                <div className='flex flex-wrap p-2 h-[150px] overflow-y-auto no-scrollbar'>
                  {theme.map((theme) => (
                    <div key={theme} >
                      <label className={(filters.theme.includes(theme) ? `text-secondary hover:text-white` : 'hover:text-secondary cursor-pointer text-white') + ` mx-1 capitalize`} htmlFor={theme}>
                        {theme}
                      </label>
                      <input
                        type="checkbox"
            ...
            <div>
              <div className='flex gap-2 items-center cursor-pointer' onClick={handlePaceDrop} ref={paceRef}>
                <h3 className='text-xl relative'>Paces</h3>
                <Image src={drop} width={15} height={10} alt='drop' />
              </div>
              <div className={(paceDrop ? `` : `hidden`) + ` flex flex-col gap-1 border-secondary absolute z-30 border-[1px] rounded-md py-2 bg-background 2xl:w-[250px] xl:w-[200px] lg:w-[200px] norm:w-[200px] md:w-[150px] sm:w-[150px] w-[100px] xs:w-[100px]`}>
                <div className='border-b-[1px] h-[47px] border-secondary pb-2 px-2 flex gap-2 py-1 overflow-x-auto no-scrollbar'>
                  {filters.pace.map((p) => (
                    <h2 key={p} className='border-secondary capitalize text-secondary border-[1px] rounded-md p-1'>{p}</h2>
                ...
                <div className='flex flex-wrap p-2 overflow-y-auto no-scrollbar'>
                  {pace.map((pace) => (
                    <div key={pace} >
                      <label className={(filters.pace.includes(pace) ? `text-secondary hover:text-white` : 'hover:text-secondary cursor-pointer text-white') + ` mx-1 capitalize`} htmlFor={pace}>
                        {pace}
                      </label>
                      <input
                        type="checkbox"
            ...
            <div>
              <div className='flex gap-2 items-center cursor-pointer' onClick={handleFormatDrop} ref={formatRef}>
                <h3 className='text-xl relative'>Formats</h3>
                <Image src={drop} width={15} height={10} alt='drop' />
              </div>
              <div className={(formatDrop ? `` : `hidden`) + ` flex flex-col gap-1 border-secondary absolute z-20 border-[1px] rounded-md py-2 bg-background 2xl:w-[250px] xl:w-[200px] lg:w-[200px] norm:w-[200px] md:w-[150px] sm:w-[150px] w-[100px] xs:w-[100px]`}>
                <div className='border-b-[1px] h-[47px] border-secondary pb-2 px-2 flex gap-2 py-1 overflow-x-auto no-scrollbar'>
                  {filters.format.map((f) => (
                    <h2 key={f} className='border-secondary capitalize text-secondary border-[1px] rounded-md p-1'>{f}</h2>
                ...
                <div className='flex flex-wrap p-2 overflow-y-auto no-scrollbar'>
                  {format.map((format) => (
                    <div key={format} >
                      <label className={(filters.format.includes(format) ? `text-secondary hover:text-white` : 'hover:text-secondary cursor-pointer text-white') + ` mx-1 capitalize`} htmlFor={format}>
                        {format}
                      </label>
                      <input
                        type="checkbox"
            ...
            <div>
              <div className='flex gap-2 items-center cursor-pointer' onClick={handleLangDrop} ref={langRef}>
                <h3 className='text-xl relative'>Languages</h3>
                <Image src={drop} width={15} height={10} alt='drop' />
              </div>
              <div className={(langDrop ? `` : `hidden`) + ` flex flex-col gap-1 border-secondary absolute z-10 border-[1px] rounded-md py-2 bg-background 2xl:w-[250px] xl:w-[200px] lg:w-[200px] norm:w-[200px] md:w-[150px] sm:w-[150px] w-[100px] xs:w-[100px]`}>
                <div className='border-b-[1px] h-[47px] border-secondary pb-2 px-2 flex gap-2 py-1 overflow-x-auto no-scrollbar'>
                  {filters.language.map((l) => (
                    <h2 key={l} className='border-secondary capitalize text-secondary border-[1px] rounded-md p-1'>{l}</h2>
                  ))}
                </div>
                <div className='flex flex-wrap p-2 h-[150px] overflow-y-auto no-scrollbar'>
                  {language.map((language) => (
                    <div key={language} >
                      <label className={(filters.language.includes(language) ? `text-secondary hover:text-white` : 'hover:text-secondary cursor-pointer text-white') + ` mx-1 capitalize`} htmlFor={language}>
                        {language}
                      </label>
                      <input
                        type="checkbox"
            ...
            <div>
              ...
              <div className='flex flex-nowrap gap-1' >
                <input
                  type="number"
                  placeholder="Min"
                  onChange={(e) => handleRatingChange(e.target.value, filters.rating[1])}
                ...
                <input
                  type="number"
                  placeholder="Max"
                  onChange={(e) => handleRatingChange(filters.rating[0], e.target.value)}
                  className='w-[75px] bg-background border-secondary border-[1px] rounded-md p-1 text-secondary outline-none'
                  max={5}
                  min={0}
                />
            ...
            <div>
              ...
              <div className='flex flex-row gap-1 flex-wrap'>
                <input
                  ...
                  onChange={(e) => handlePublishDateChange(e.target.value, filters.publishDate[1])}
                ...
                <input
                  ...
                  onChange={(e) => handlePublishDateChange(filters.publishDate[0], e.target.value)}
                  className=' bg-background border-secondary border-[1px] rounded-md p-1 text-secondary outline-none'
            ...
            <div>
              <button onClick={handleSubmit} className='bg-secondary w-full rounded-md my-2 text-2xl text-white'>
                Search
              </button>
            </div>
          </div>
          <div>
            <div>
              <label htmlFor="sortOptions" className='text-xl'>Sort By: </label>
              <select onChange={handleSortChange} value={sort} name="sort" id="sortOptions" className='w-full h-[40px] text-lg bg-background border-secondary border-[1px] rounded-md p-1 outline-none text-secondary' >
            ...
            <div className='flex gap-2 mt-3 items-center'>
              <label htmlFor="orderOptions" className='text-xl'>Order by: </label>
              <div id="orderOptions" className='flex gap-4 cursor-pointer'>
                <div onClick={toggleUp} onMouseEnter={() => setHoveringUp(true)} onMouseLeave={() => setHoveringUp(false)}>
                  <svg
                    fill={(order === 'asc') ? (hoveringUp ? "#ffffff" : "#5D68B0") : (hoveringUp ? "#5D68B0" : "#ffffff")}
                    height="25px"
                    width="25px"
                    version="1.1"
                    id="Layer_1"
                ...
                <div onClick={toggleDown} onMouseEnter={() => setHoveringDown(true)} onMouseLeave={() => setHoveringDown(false)}>
                  <svg
                    fill={(order === 'desc') ? (hoveringDown ? "#ffffff" : "#5D68B0") : (hoveringDown ? "#5D68B0" : "#ffffff")}
                    height="25px"
                    width="25px"
                    version="1.1"
                    id="Layer_1"
