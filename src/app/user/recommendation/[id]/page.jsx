...
export default function Recommendations() {
  ...
  async function getRecommendations() {
    ...
    const weights = calculateNormalizedWeights(); // Use normalized weights for the model
    setLoading(true);
    ...
    try {
      const response = await fetch("/api/recommend", {
        ...
        body: JSON.stringify({
          userInput: constructedInput, // Use the constructed input string
          weights,
        }),
      });
      ...
      if (response.ok) {
        ...
        if (bookDetailsResponse.ok) {
          ...
          setRecommendations(bookDetailsData.books); // Set detailed book data
  
          // Send recommendations to the user's bookList in MongoDB
          const userId = id; // Extract userId from the URL
          const bookIds = data.recommendations.map((rec) => rec._id);
  
          await fetch("/api/add-recommendations", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
